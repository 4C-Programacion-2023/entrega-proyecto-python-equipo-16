...
def hay_empate(tablero):   #si hay empate
    for fila in tablero:
        for casilla in fila:
            if casilla == " ":
                return False
...
def jugar_tres_en_linea():
    while True:
        ...
        while not juego_terminado:
            ...
            try:
                fila = int(input("Ingrese el número de fila (0-2): "))   #selección de donde poner la "ficha"
                ...
                if tablero[fila][columna] == " ":   #posibilidades de ubicación
        ...
        opcion = input(
            "¿Quiere jugar de vuelta? Ingrese 1 para volver a jugar, si no lo desea, ingrese otro parámetro: ")   #volver a jugar
        if opcion != "1":
